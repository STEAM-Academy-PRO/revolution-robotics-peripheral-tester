...
class MotorPortWrapper(Wrapper):
    ...
    @property
    def speed(self) -> int:
        return self._motor.driver.speed
    ...
    def move(self, direction, amount, unit_amount, limit, unit_limit) -> None:
        ...
        move_motor_command_map = {
            ...
            MotorConstants.UNIT_SEC: {
                MotorConstants.UNIT_SPEED_RPM: {
                    MotorConstants.DIRECTION_FWD: lambda: self._motor.driver.set_speed(
                        limit
                    ),
                    MotorConstants.DIRECTION_BACK: lambda: self._motor.driver.set_speed(
                        -limit
                    ),
                },
                MotorConstants.UNIT_SPEED_PWR: {
                    MotorConstants.DIRECTION_FWD: lambda: self._motor.driver.set_power(
                        limit
                    ),
                    MotorConstants.DIRECTION_BACK: lambda: self._motor.driver.set_power(
                        -limit
                    ),
                },
            },
        ...
        motor_control_command = move_motor_command_map[unit_amount][unit_limit][
            direction
        ]
        ...
        with self.try_take_resource(_interrupted) as resource:
            if resource:
                ...
                if unit_amount == MotorConstants.UNIT_DEG:
                    ...
                    if (
                        awaiter
                    ):  # can be None if resource is interrupted by another script
                        awaiter.wait()
                    ...
                    try:
                        ...
                        resource.run_uninterruptable(
                            partial(self._motor.driver.set_power, 0)
                        )
...
class DriveTrainWrapper(Wrapper):
    ...
    def set_speed(
        self, direction, speed, unit_speed=MotorConstants.UNIT_SPEED_RPM
    ) -> None:
...
class LineDriver:
    ...
    def __init__(
        self,
        drivetrain: "DriveTrainWrapper",
        color_reader: "RobotWrapper",
        line_color: str,
    ) -> None:
        self.__drivetrain = drivetrain
    ...
    def __getitem__(self, item: Union[int, str]) -> PortWrapper:
        if type(item) is str:
            ...
            if item in self._alias_map.keys():
                ...
                raise KeyError(
                    f"key '{item}' not found in alias map. Available keys: {key_list}"
                )
...
class RobotWrapper:
    ...
    def __init__(
        ...
        motor_wrappers = [
            MotorPortWrapper(script, port, resources[f"motor_{port.id}"])
        ...
        self._drivetrain = DriveTrainWrapper(
            script, robot.drivetrain, resources["drivetrain"]
        )
    ...
    def read_rgb_sensor_data(self) -> list[ColorData]:
        ...
        return [
            ...
            rgb_to_hsv_gray(
                sensor_data.right.r, sensor_data.right.g, sensor_data.right.b
            ),
            rgb_to_hsv_gray(
                sensor_data.middle.r, sensor_data.middle.g, sensor_data.middle.b
            ),
        ]
